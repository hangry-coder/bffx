package specs

import (
	bffxtest "github.com/hangry-coder/bffx/pkg/testing"
	"testing"
)

func TestAppPermission(t *testing.T) {
	bffxtest.SetT(t)
	env := bffxtest.SetupEnvironment("..")

	bffxtest.Describe("AppPermission Resource", func() {
		bffxtest.It("should build a valid AppPermission from blueprint", func() {
			data := env.Factory.Build("AppPermissionDefault", nil)
			bffxtest.Expect(data).ToNotEqual(nil)
		})

		bffxtest.It("should create a AppPermission in the store", func() {
			record, err := env.Factory.Create("AppPermissionDefault", nil)
			bffxtest.Expect(err).ToEqual(nil)
			bffxtest.Expect(record["id"]).ToNotEqual("")
		})
	})
}
